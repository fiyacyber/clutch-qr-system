...
function errorResponse(req: NextRequest, error: string, status = 400) {
...
export async function POST(req: NextRequest) {
  ...
  if (!user || !customer) return errorResponse(req, "Unauthorized", 401);

  const form = await req.formData();
  const action = String(form.get("action") || "").trim();
  const profileId = String(form.get("profile_id") || "").trim();

  if (!profileId) return errorResponse(req, "Profile is required.", 400);
  ...
  if (!profile) return errorResponse(req, "Profile not found.", 404);
  ...
  if (action === "create") {
    ...
    if (!label || !url) {
      return errorResponse(req, "Label and URL are required.", 400);
    }
    ...
    if (error) {
      ...
      return errorResponse(req, "Failed to add link.", 500);
    }
  }
  ...
  if (action === "update") {
    ...
    if (error) {
      ...
      return errorResponse(req, "Failed to update link.", 500);
    }
  }
  ...
  if (action === "delete") {
    ...
    if (error) {
      ...
      return errorResponse(req, "Failed to delete link.", 500);
    }
  }
