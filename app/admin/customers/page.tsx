...
export default async function AdminCustomersPage({ searchParams }: AdminPageProps) {
    ...
    <DashboardShell isAdmin>
      <main className={`${styles.page} container admin-page`}>
        ...
        <section className="card">
          <form className="admin-search-form" action="/admin/customers" method="get">
            <input
              className="input"
              name="q"
            ...
            {searchQuery ? <Link className="btn ghost" href="/admin/customers">Clear</Link> : null}
          </form>
        </section>
