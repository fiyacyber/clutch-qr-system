...
export default async function AdminPrintOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [orderResult, filesResult, proofsResult, activityResult] = await Promise.all([
  ...
  if (filesResult.error || proofsResult.error || activityResult.error) throw new Error("Unable to load print operations details.");
