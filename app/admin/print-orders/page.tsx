...
export default async function AdminPrintOrdersPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <DashboardShell isAdmin>
    <main className="container admin-page">
      ...
      <div className="table-scroll"><table><thead><tr><th>Order</th><th>Customer</th><th>Product</th><th>SKU / material</th><th>Qty</th><th>Tracking</th><th>Campaign</th><th>Artwork method</th><th>Artwork</th><th>Proof</th><th>Production</th><th>Fulfillment</th><th>Provisioning</th><th>QR</th><th>Attention</th><th>Created</th></tr></thead>
        <tbody>{(data || []).map((item: any) => <tr key={item.id}><td><Link href={`/admin/print-orders/${item.id}`}>{item.shopify_order_number || item.shopify_order_id}</Link></td><td>{item.customer_name || item.customer_email || "Guest"}</td><td>{item.product_title}{item.variant_title ? ` — ${item.variant_title}` : ""}</td><td>{item.sku || "—"}<br />{item.material_type}</td><td>{item.quantity}</td><td>{item.tracking_mode}</td><td>{item.campaign_name || "—"}</td><td>{item.artwork_method || "—"}{item.reorder_reference ? <><br />{item.reorder_reference}</> : null}</td><td>{item.artwork_status}</td><td>{item.proof_status}</td><td>{item.production_status}</td><td>{item.fulfillment_status}</td><td>{item.provisioning_status}</td><td>{item.print_qr_provisionings?.[0]?.provisioning_status || "—"}</td><td>{item.attention_reason || "—"}</td><td>{new Date(item.created_at).toLocaleString()}</td></tr>)}</tbody>
      </table></div>
